...
Contrastive Learning Example using the framework SimCLR & Ensemble
...
def main():
    ...
    save_file = './results/simclr_ensemble/'
    use_scheduler = True
    ...
    # Load the data
    contrastive_set = datasets.MNIST('./data', train=True, transform=TwoCropTransform(contrastive_transform))
    train_set = datasets.MNIST('./data/', train=True, download=True, transform=train_transform)
    test_set = datasets.MNIST('./data/', train=False, download=True, transform=valid_transform)
    ...
    # Define data loaders
    contrastive_loader = torch.utils.data.DataLoader(contrastive_set, batch_size=256, sampler=SubsetRandomSampler(indices=train_idx))
    ...
    for model_name in model_names:
        ...
        save_model(model, optimizer, num_epochs, save_file + 'encoder_' + model_name + '.pth')
        ...
        ckpt = torch.load(save_file + 'encoder_' + model_name + '.pth', map_location='cpu')
    ...
    for model_name in model_names:
        ...
        y_hat_valid = classifier(model.encoder((x_valid)))
        y_hat_test = classifier(model.encoder((x_test)))
        ...
        # Store the outputs
        trained_models[model_name] = torch.nn.Sequential(model, classifier)
        metrics[model_name] = {
