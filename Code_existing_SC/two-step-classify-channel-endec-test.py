...
class Channel_Endec(keras.Model):
    ...
    # def train(self, train_x):
    #     samples, labels = train_x[0], train_x[1]
    #     loss = model.train_on_batch(samples, labels)
    #     x_hat = model.predict_on_batch(samples)
    #     acc = tf.keras.metrics.categorical_accuracy(labels, x_hat)
    #     print('model acc is :')
    #     print(tf.reduce_mean(acc))
    #     print('model loss is :')
...
if __name__ == '__main__':
    ...
    model.encoder.save('/home/Attack_sc/models/classification-task/channel_encoder', save_format='tf')
    model.decoder.save('/home/Attack_sc/models/classification-task/channel_decoder', save_format='tf')
