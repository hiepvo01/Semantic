...
class Channel_Endec(keras.Model):
    ...
    def training_log(self, train_x, test, train_loss, x_hat, step, epoch):
        ...
        return [epoch, step + (1 + N_TRAIN_BATCHES) * epoch + 1, train_loss, tf.reduce_mean(train_acc), test_loss,
    ...
    def attacking_with_budget(self, message):
        # index = tf.argmax(message, axis=1)
        # # mal = np.random.randint(low=-1, high=1, size=message.shape, dtype='l')

        malicious = attacking_matrix(m, message)
        # malicious = attacking_matrix_gradients(m, message)
    ...
    def attack_forward_train(self, train_x, test_db, step, epoch):
        ...
        with tf.GradientTape() as en_tape:
            ...
            # z_channel = AWGN(z_channel, 15, seed=10)
            # z_channel = awgn(z_channel, 15)
            z_channel_attacked = self.attacking_with_budget(z_channel)
    ...
    def attacking_with_budget_back(self, message):
        # index = tf.argmax(message, axis=1)
        # # mal = np.random.randint(low=-1, high=1, size=message.shape, dtype='l')

        # malicious = attacking_matrix(m, message)
        M = []
        for message_silde in message:
            malicious_temp = attacking_matrix_gradients(m, message_silde)
            # M = message_silde * malicious_temp
            M.append(message_silde * malicious_temp)
        # malicious = np.ones_like(message) * -1
    ...
    def attack_backward_train(self, train_x, test_db, step, epoch):
        ...
        de_gradients = de_tape.gradient(decoder_loss, self.decoder.trainable_variables)
        # malicious = np.ones_like(en_gradients) * -1
        # en_gradients *= malicious
        en_gradients = self.attacking_with_budget_back(en_gradients)
...
# message=[1024, 2048] during attacking forward,
def init_attacking_matrix(m, message=[1024, 2048]):
    lenth = message[0] * message[1]

    negative_len = int(lenth * m)
    negative = np.ones((negative_len,)) * -1
    ones = np.ones((lenth - negative_len,))

    values = np.concatenate((ones, negative), )

    mal_martix = np.random.choice(values, lenth, replace=False).reshape(message)

    return mal_martix


# message=[1024, 2048] during attacking forward,
def attacking_matrix(m, message):
    lenth = message.shape[0] * message.shape[1]
    negative_len = int(lenth * m)
    negative = np.ones((negative_len,)) * -1
    ones = np.ones((lenth - negative_len,))
    ...
    mal_martix = np.random.choice(values, lenth, replace=False).reshape(message.shape)

    return mal_martix


def attacking_matrix_gradients(m, message):
    lenth = 0
    if len(message.shape) == 1:
        lenth += message.shape[0]
    else:
        lenth += message.shape[0] * message.shape[1]
        # lenth = message[0] * message[1]
    negative_len = int(lenth * m)
    ...
    # mal_shape = []
    if len(message.shape) == 1:
        mal_shape = message.shape[0]
    else:
        mal_shape = [message.shape[0], message.shape[1]]
    mal_martix = np.random.choice(values, lenth, replace=False).reshape(mal_shape)
    return mal_martix
...
if __name__ == '__main__':
    ...
    m = 0.3
    init_mal_matrix = init_attacking_matrix(m)
    ### Train the model
    for epoch in range(50):
        for step, x in enumerate(train_db):
            ...
            save.to_csv('/home/Attack_sc/models_budget/newlog-back_m%s.csv' % m)
    ...
    model.encoder.save('/home/Attack_sc/models_budget/CIFAR-classification-task-newback_m%s/channel_encoder' % m,
                       save_format='tf')
    model.decoder.save('/home/Attack_sc/models_budget/CIFAR-classification-task-newback_m%s/channel_decoder' % m,
                       save_format='tf')

    E = load_model('/home/Attack_sc/models_budget/CIFAR-classification-task-newback_m%s/channel_encoder' % m)
    E.trainable = False
    D = load_model('/home/Attack_sc/models_budget/CIFAR-classification-task-newback_m%s/channel_decoder' % m)
    D.trainable = False
