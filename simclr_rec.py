...
def plot_ae_outputs(encoder,decoder, test_dataset, source, epoch,n=10):
    ...
    plt.savefig('./figures/'+source+'/SNR25/' + source + '_epoch_' +str(epoch) +'.pdf', bbox_inches='tight')
...
def loop(trainset, testset, trainloader, testloader, source):
    ...
    simclr_model.convnet.load_state_dict(
        torch.load('./results/simclrSTL-512.pt')
    )
    ...
    for epoch in range(num_epochs): #(loop for every epoch)
        ...
        df.to_csv('./results/' + source + '/' + source + '_SIMCLR_SNR25.csv')
            
        
    save_path = './results/'+ source +'/' + source +  'encoderSNR25.pt'
    # torch.save(model.state_dict(), save_path)
    save_path = './results/'+ source +'/' + source +  'decoderSNR25.pt'
